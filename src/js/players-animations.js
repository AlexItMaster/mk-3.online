import '../scss/players-animations.scss';

/** Підключення модулів. */
import { ANIMATE } from './modules/constants';
import {camelize, createNewTag} from './modules/utils';
import { getImg } from './modules/logic';
    ...
    for(let key in ANIMATE) {
        if (Array.isArray(ANIMATE[key]) && ANIMATE[key].length) {
            ...
            const $imgAvatar = createNewTag({
                tagName: 'img',
                attrList: {
                    src: getImg({type: 'avatars', name: camelize(key, true)}),
            ...
            ANIMATE[key].forEach(item => {  
                ...
                const $img = createNewTag({
                    tagName: 'img',
                    attrList: {
                        src: getImg({type: item, name: camelize(key, true)}),
                        alt: item,
                    },            
                });
